...
export function TopMenu({ theme, onThemeChange }: TopMenuProps) {
  ...
  return (
    <div className="fixed right-4 top-4 z-50">
      <button
        className="cursor-pointer grid h-11 w-11 place-items-center rounded-full bg-[var(--button-color)] text-lg font-bold text-white shadow-lg transition hover:-translate-y-0.5 hover:brightness-110"
      ...
      {open && (
        <div className="absolute right-0 top-14 grid w-64 gap-4 rounded-[22px] border-2 border-[var(--outline-color)] bg-[var(--card-color)] p-4 text-[var(--text-color)] shadow-2xl pt-6 pb-7">
          ...
          <button
            className="cursor-pointer rounded-full border-2 border-[var(--outline-color)] bg-transparent px-4 py-3 font-black text-[var(--text-color)] transition hover:-translate-y-0.5 hover:bg-[color-mix(in_srgb,var(--outline-color)_18%,transparent)]"
            onClick={resetColors}
          >
            Reset colors
          </button>
